          ...
          <el-form :model="form" :rules="rules" ref="form" size="default">
            ...
            <div class="login-form">
              ...
              <el-form-item prop="password">
                <el-input type="password" v-model="form.password" :placeholder="$t('login.password')" maxlength="30" />
              </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        username: [{ required: true, message: this.$tm("commons.validate.input", "login.username"), trigger: "blur" }],
        password: [
        ...
        code: [{ required: true, message: this.$tm("commons.validate.input", "login.captcha"), trigger: "blur" }],
      },
      msg: "",
      redirect: undefined,
      otherQuery: {},
      ...
      forgetRules: {
        username: [{ required: true, message: this.$tm("commons.validate.input", "login.username"), trigger: "blur" }],
        email: [{ required: true, message: this.$tm("commons.validate.input", "login.email"), trigger: "blur" }],
      },
      loadingPage: false,
    }
  },
  ...
  methods: {
    ...
    getSystemName() {
      if (store.getters.theme) {
        ...
        this.loginImage = store.getters.theme.loginImage
      } else {
        this.$store.dispatch("theme/getThemeInfo").then((data) => {
          this.systemName = data.systemName
          this.loginImage = data.loginImage
        })
      }
    },
    checkErrorNum() {
